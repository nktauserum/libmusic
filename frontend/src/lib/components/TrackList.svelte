<script lang="ts">
    ...
    export let tracks: Track[] | null = null;

    function playTrack(track_position: number) {
        if (tracks) {
            playerStore.play(tracks, track_position);
        }
    }
</script>
...
<div class="tracks">
    {#if tracks !== null}
        {#each tracks as track, i}
            <button class="track" on:click={() => playTrack(i)} aria-label="track">
